...
// 3. მოკლევადიანი კარტის ზუსტი ჩანაცვლება TouchableOpacity-დ
code = code.replace(
  /<View style=\{\[\s*styles\.card,\s*\{\s*marginTop:\s*10,\s*padding:\s*12\s*\}\s*\]\}>\s*<View style=\{\{\s*flexDirection:\s*'row',\s*alignItems:\s*'center'\s*\}\}\>/,
...
// ვცვლით შესაბამის დამხურავ ტეგს მოკლევადიანი კარტისთვის
code = code.replace(
  /(<Ionicons name="flash-outline"[^>]*>[\s\S]*?<\/Text>\s*<\/View>\s*)<\/View>/,
  '$1</TouchableOpacity>'
);

// 4. გრძელვადიანი კარტის ზუსტი ჩანაცვლება TouchableOpacity-დ
code = code.replace(
  /<View style=\{\[\s*styles\.card,\s*\{\s*marginTop:\s*14,\s*padding:\s*12\s*\}\s*\]\}>\s*<View style=\{\{\s*flexDirection:\s*'row',\s*alignItems:\s*'center'\s*\}\}\>/,
  '<TouchableOpacity onPress={() => setIsLongModalVisible(true)} activeOpacity={0.8} style={[styles.card, { marginTop: 14, padding: 12 }]}>\n                <View style={{ flexDirection: \'row\', alignItems: \'center\' }}>'
);

code = code.replace(
  /(<Ionicons name="planet-outline"[^>]*>[\s\S]*?<\/Text>\s*<\/View>\s*)<\/View>/,
  '$1</TouchableOpacity>'
);
...
console.log('✅ ზუსტი პატჩი წარმატებით დასრულდა!');
