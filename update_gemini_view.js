...
const tabHeader = "activeTab === 'ტყუპები' && (";
const idx = code.indexOf(tabHeader);

if (idx !== -1) {
  // ვპოულობთ სად მთავრდება ეს ბლოკი (შემდეგი ჩანართი ან ძირითადი ბლოკის დასასრული)
  let nextTab = code.indexOf("activeTab ===", idx + 20);
  if (nextTab === -1) nextTab = code.indexOf("activeTab===", idx + 20);
  
  if (nextTab !== -1) {
    const newContent = `activeTab === 'ტყუპები' && (
    ...
    // ვპოულობთ მიმდინარე ჩანართის დასაწყის ხაზს
    ...
    console.log('⚠️ საზღვრები ვერ მოიძებნა ზუსტად.');
  }
} else {
  console.log('❌ ტყუპების ჩანართი ვერ მოიძებნა.');
}
