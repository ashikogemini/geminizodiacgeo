...
// 3. ვპოულობთ და ვცვლით მოკლევადიანი კარტის საწყის View-ს TouchableOpacity-დ
code = code.replace(
  'style={[styles.card, { marginTop: 10, padding: 12 }]}',
  'onPress={() => setIsShortModalVisible(true)} activeOpacity={0.8} style={[styles.card, { marginTop: 10, padding: 12 }]} componentType="TouchableOpacity"'
);
// ვცვლით View-ს TouchableOpacity-დ ამ კარტისთვის
code = code.replace(
  /<View style=\{\[styles\.card, \{ marginTop: 10, padding: 12 \}\]\} componentType="TouchableOpacity"/,
  '<TouchableOpacity onPress={() => setIsShortModalVisible(true)} activeOpacity={0.8} style={[styles.card, { marginTop: 10, padding: 12 }]} '
);

// 4. ვპოულობთ და ვცვლით გრძელვადიანი კარტის საწყის View-ს TouchableOpacity-დ
code = code.replace(
  'style={[styles.card, { marginTop: 14, padding: 12 }]}',
  'onPress={() => setIsLongModalVisible(true)} activeOpacity={0.8} style={[styles.card, { marginTop: 14, padding: 12 }]} componentType="TouchableOpacity"'
);
code = code.replace(
  /<View style=\{\[styles\.card, \{ marginTop: 14, padding: 12 \}\]\} componentType="TouchableOpacity"/,
  '<TouchableOpacity onPress={() => setIsLongModalVisible(true)} activeOpacity={0.8} style={[styles.card, { marginTop: 14, padding: 12 }]} '
);

// 5. მოდალური ფანჯრები რენდერის ბოლოში
const modalsJSX = `
...
console.log('✅ ფინალური პატჩი წარმატებით დასრულდა!');
