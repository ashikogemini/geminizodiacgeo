...
// 3. უსაფრთხოდ ვპოულობთ და ვცვლით მხოლოდ სათაურის ველებს TouchableOpacity-დ
code = code.replace(
  /<View style=\{\[\s*styles\.card,\s*\{\s*marginTop:\s*10,\s*padding:\s*12\s*\}\s*\]\}>/g,
  '<TouchableOpacity onPress={() => setIsShortModalVisible(true)} activeOpacity={0.8} style={[styles.card, { marginTop: 10, padding: 12 }]}>'
);

code = code.replace(
  /<View style=\{\[\s*styles\.card,\s*\{\s*marginTop:\s*14,\s*padding:\s*12\s*\}\s*\]\}>/g,
  '<TouchableOpacity onPress={() => setIsLongModalVisible(true)} activeOpacity={0.8} style={[styles.card, { marginTop: 14, padding: 12 }]}>'
);

// ვცვლით შესაბამის დამხურავ ტეგებს TouchableOpacity-თ იმ ორი კარტისთვის, სადაც flash-outline და planet-outline-ია
// მარტივად: ვპოულობთ კარტის შიგთავსის ბოლოს და ვასწორებთ
code = code.replace(
  /(<Ionicons name="flash-outline"[^>]*>[\s\S]*?)<\/View>\s*<\/View>\s*<\/View>/g,
  '$1</View></View></TouchableOpacity>'
);

code = code.replace(
  /(<Ionicons name="planet-outline"[^>]*>[\s\S]*?)<\/View>\s*<\/View>\s*<\/View>/g,
  '$1</View></View></TouchableOpacity>'
);

// 4. მოდალური ფანჯრები რენდერის ბოლოში
const modalsJSX = `
...
console.log('✅ სუფთა პატჩი წარმატებით დასრულდა!');
