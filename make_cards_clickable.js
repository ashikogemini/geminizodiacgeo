...
if (!code.includes('Modal')) {
  code = code.replace(/import \{([^}]+)\} from "react-native";/, 'import {$1, Modal, ScrollView} from "react-native";');
}

// 2. ვამატებთ State-ებს
...
// 3. ახალი, დაკლიკებადი ბლოკები (ისრებით)
const shortTermClickable = `{/* მოკლევადიანი ასპექტები */}
...
const longTermClickable = `{/* გრძელვადიანი ასპექტები */}
...
// 4. მოკლევადიანი ბლოკის ზუსტი ჩანაცვლება
let startShort = code.indexOf('{/* მოკლევადიანი ასპექტები */}');
if (startShort !== -1) {
    let firstClose = code.indexOf('</View>', startShort);
    let secondClose = code.indexOf('</View>', firstClose + 7);
    if (firstClose !== -1 && secondClose !== -1) {
         let block = code.substring(startShort, secondClose + 7);
         if (block.includes('<View') && !block.includes('<TouchableOpacity')) {
             code = code.substring(0, startShort) + shortTermClickable + code.substring(secondClose + 7);
         }
    }
}

// 5. გრძელვადიანი ბლოკის ზუსტი ჩანაცვლება
let startLong = code.indexOf('{/* გრძელვადიანი ასპექტები */}');
if (startLong !== -1) {
    let firstClose = code.indexOf('</View>', startLong);
    let secondClose = code.indexOf('</View>', firstClose + 7);
    if (firstClose !== -1 && secondClose !== -1) {
         let block = code.substring(startLong, secondClose + 7);
         if (block.includes('<View') && !block.includes('<TouchableOpacity')) {
             code = code.substring(0, startLong) + longTermClickable + code.substring(secondClose + 7);
         }
    }
}

// 6. მოდალური ფანჯრების დამატება ბოლოში
const modalsJSX = `
...
if (!code.includes('⚡ მოკლევადიანი ასპექტები') && code.includes('export default function App')) {
  const lastIndex = code.lastIndexOf('</View>');
  if (lastIndex !== -1) {
    code = code.substring(0, lastIndex) + modalsJSX + code.substring(lastIndex);
  }
}

fs.writeFileSync('App.js', code, 'utf8');
console.log('✅ ფინალური, უშეცდომო ჩანაცვლება წარმატებით დასრულდა!');
